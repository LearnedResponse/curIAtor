...
> **Status:** retired to completed as of 2026-07-04. The local collection, seeded loop, static-publish
> path, and strict fresh-clone browser preflight are complete through `galleries/curiator-phylogenetics@38cefee`;
> only Pages enable/run/URL wiring remains external. Scaffolded 2026-07-02 in nested repo
> `galleries/curiator-phylogenetics`.
...
3. **curiator's first client-side-compute (browser-WASM / Pyodide) collection** — the app
   ...
   [math-geometry-collection](math-geometry-collection.md).
...
- `curiator stats --json`: 8 cycles, 8 replied/done cycles, 8 curator commits, latest feedback-loop
  receipt `b1b3586`.
- `curiator release-preflight --gallery curiator-phylogenetics --fresh-clone --strict --browser-smoke --json`:
  passing at `38cefee`; the temp clone runs both app smoke hooks, opens both apps through the shell in
  headless Brave, and finds no tracked machine-local paths.
...
- `galleries/curiator-phylogenetics` is initialized as its own git repo with seed commit `713b39e`;
  the current verified head is `38cefee`.
...
[math-geometry-collection](math-geometry-collection.md)) and as an evaluation collection for
[`zenodo-paper.md`](../zenodo-paper.md). Build
