...
> scaffolds (dash/static/python/node/flask/rust/react/svelte/vue/streamlit/gradio) are in the runner, and the **non-Dash proof now exists**:
...
> template hardening beyond the first React/Svelte/Vue/Flask/Rust/Streamlit/Gradio scaffolds, heavier Docker/Compose
...
flask / rust / react / svelte / vue / streamlit / gradio), each emitting a directory + the right `mount`
...
3. **Scaffold templates** (`curiator init-app --template …`) — Node's dependency-light HTTP scaffold,
   Flask's server-rendered scaffold, and Rust's dependency-light HTTP scaffold are available for small server-side prototypes; first pass landed
