...
> explorer (`tinnik_dash`). First curator receipt landed at `bb597ed` on `curiator/auto`, adding
> the compact "what is D?" explainer to the static CF ternary. Captured 2026-06-30; first scaffold
> landed 2026-07-02.
...
6. **Run the loop** — `curiator seed && curiator watch`: the curator evolves each app
   ...
   gallery growing from paper-figures into an explorable public companion. First loop receipt
   landed at `bb597ed`; seven seeded reviewer items remain open.
...
- `curiator stats --json`: 8 cycles, 1 replied/done cycle, 1 curator commit, latest `bb597ed`.
- `curiator release-preflight --gallery curiator-phylogenetics --fresh-clone --json`: passing at
  `bb597ed`; the temp clone runs both app smoke hooks and finds no tracked machine-local paths.
...
- `galleries/curiator-phylogenetics` is initialized as its own git repo with seed commit `713b39e`;
  the current `curiator/auto` head is `bb597ed`.
