...
def test_general_collection_commit_commits_nested_imported_app_first(cfg, collection):
    ...
    subprocess.run([
        "git", "-c", "user.name=Seeder", "-c", "user.email=seed@test.local",
        "commit", "-q", "-m", "imported app seed",
    ], cwd=appdir, check=True, capture_output=True)
    ...
    fid = ledger.save_entry(cfg, "__general__", comment="create a new curiator app for imported", ts="t0")
    ledger.add_system_note(cfg, "__general__", "Imported and mounted the app.", reply_to=[fid], ts="t1")
    ledger.set_status(cfg, "__general__", [fid], "done")

    res = gitmem.commit_run(cfg, "__general__", fid, status="done", note_text="Imported and mounted the app.")
    ...
    assert "gallery.yaml" in parent_files
